...
``repocharter revert`` can restore it.
"""
...
class Action:
    ...
    target: str
    detail: str
    reversible: str = "quarantined; `repocharter revert` restores"
    ...
    if m2.policy_empty:
        plan.judgment.append(Decision(
            ...
            evidence=[
                ...
                "run `repocharter policy scaffold` to harvest a draft from this repo's existing "
...
def render(plan: Plan) -> str:
    lines = [
        ...
        "",
        f"Stage: **{plan.stage}**",
        f"Generated by `repocharter migrate`. Mechanical actions are applied with `--apply`;",
    ...
    lines += [
        ...
        "with a manifest; `repocharter revert` restores them.",
        "",
    ]
    return "\n".join(lines)
        ...
        if action.kind == "neutralise-nested-agents":
            ...
            for name in CANONICAL_FILES:
                ...
                if name == "pitfalls.md" and template.exists():
                    ...
                    target.write_text(
                        f"# {name.replace('.md', '')}\n\n"
                        f"<!-- Created empty by `repocharter migrate`. See the migration plan and\n"
